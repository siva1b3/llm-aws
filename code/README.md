...
- `Phase 1 / Level 05 — complete. LLM core now returns vectors [N_tok × d] (last hidden state) instead of a token ID. New ""to token"" step (opaque) holds final norm + LM head + take-last-row + softmax + argmax; these will be peeled out in Levels 06–11.`
- `Phase 1 / Level 06 — complete. `to token` opaque box split into LM head (Step 4) + pick (Step 5). LM head is a matmul `vectors · {H}` producing `logits [N_tok × vocab_size]`; `{H}` named but still opaque (opens at Level 07). Pick is stateless and currently bundles take-last-row + softmax + argmax (peeled out in Levels 08–10).`